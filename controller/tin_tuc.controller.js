...
class tin_tuc_controller {
    ...
    GET_DETAIL_TIN_TUC(req, res) {
        Sale_Schema.findById(req.params._id, function (err, sale) {
            tin_tuc_Schema.findById(req.params.id, function (err, tin_tuc_detail) {
                thong_bao_Schema.find(function (err, thong_bao) {
                    tin_tuc_Schema.find(function (err, list_tin_tuc) {
                        if (err) console.log(err);
                        res.render('admin/pages/tin_tuc/chi_tiet_tin_tuc', { sale, tin_tuc_detail, thong_bao, list_tin_tuc });
                    })
                })
            })
        })
    ...
    GET_DANH_SACH_TIN_TUC(req,res){
        Sale_Schema.findById(req.params._id, function (err, sale) {
            tin_tuc_Schema.find(function (err, tin_tucs) {
                thong_bao_Schema.find(function (err, thong_bao) {
                    if (err) console.log(err);
                    res.render('admin/pages/tin_tuc/danh_sach_tin_tuc', { sale, tin_tucs, thong_bao, message: req.flash('message') });
                })
            }).populate('nguoi_dang_bai').sort({ create_date: -1 })
        })
    }
    ...
    POST_TIN_TUC(req, res) {
        const new_tin_tuc = new tin_tuc_Schema({
            ...
            nguoi_dang_bai: req.body.nguoi_dang_bai
        });
        new_tin_tuc.save();
